...
def _dump_vprime(
    ...
    visual_tokens = inputs_embeds[:, visual_span, :].squeeze(0)
    vprime = F.interpolate(
        visual_tokens.float().unsqueeze(0).unsqueeze(0),
        size=(n_prime, d_prime),
        mode="bilinear",
        align_corners=False,
    ).squeeze(0).squeeze(0)
    vprime = vprime.to(torch.float16)
    torch.save(vprime, output_path)
    return output_path
...
def run_causal_tracing(
    ...
    vprime_dir = None
    if dump_vprime:
        base_output_path = representations_output or output_path
        base_dir = os.path.dirname(base_output_path)
        vprime_dir = vprime_out_dir or os.path.join(base_dir, "vprime")
        os.makedirs(vprime_dir, exist_ok=True)
    ...
    for sample in tqdm(dataset):
        ...
        if dump_representations and not representations_output:
            sample_reps = _collect_representations(
                model,
                inputs_clean,
                clean_inputs_embeds,
                token_slice,
            ...
            if dump_vprime and vprime_dir:
                vprime_path = os.path.join(vprime_dir, f"sample_{question_id}_clean_vprime.pt")
                sample_reps["vprime_path"] = _dump_vprime(
                    inputs_clean,
                    clean_inputs_embeds,
                    model.processor,
                    vprime_path,
            ...
            sample_result["representations"] = sample_reps
        if dump_representations and representation_results is not None:
            rep_reps = _collect_representations(
                model,
                inputs_clean,
                clean_inputs_embeds,
                token_slice,
            ...
            if dump_vprime and vprime_dir:
                vprime_path = os.path.join(vprime_dir, f"sample_{question_id}_clean_vprime.pt")
                rep_reps["vprime_path"] = _dump_vprime(
                    inputs_clean,
                    clean_inputs_embeds,
                    model.processor,
                    vprime_path,
        ...
        for name, cf_frames in counterfactuals.items():
            ...
            if dump_representations and not representations_output:
                ...
                cf_reps = _collect_representations(
                    model,
                    inputs_cf,
                    cf_inputs_embeds,
                    token_slice,
                ...
                if dump_vprime and vprime_dir:
                    filename = f"sample_{question_id}_{name}_vprime.pt"
                    if name == "local_swap":
                        filename = (
                            f"sample_{question_id}_{name}_"
                            f"{swap_spans[0][0]}-{swap_spans[0][1]}_"
                            f"{swap_spans[1][0]}-{swap_spans[1][1]}_vprime.pt"
                        )
                    vprime_path = os.path.join(vprime_dir, filename)
                    cf_reps["vprime_path"] = _dump_vprime(
                        inputs_cf,
                        cf_inputs_embeds,
                        model.processor,
                        vprime_path,
                ...
                cf_entry["representations"] = cf_reps
            if dump_representations and representation_results is not None:
                cf_inputs_embeds = model.get_text_model_in(cf_frames, prompt)
                cf_reps = _collect_representations(
                    model,
...
def main() -> None:
    ...
    parser.add_argument(
        "--dump_vprime",
        action="store_true",
        help="Dump downsampled visual token embeddings (V') to .pt files.",
    )
    parser.add_argument("--vprime_n", type=int, default=128, help="Output token count for V'.")
    parser.add_argument("--vprime_d", type=int, default=1024, help="Output embedding width for V'.")
    parser.add_argument(
        "--vprime_out_dir",
        default=None,
        help="Directory to save V' .pt files (default: alongside output JSON in vprime/).",
    )
